...
function applySearch(url: URL, search: Record<string, unknown>, config: ClientConfig): void {
...
function buildURL(config: ClientConfig, path: string, opts: RequestOptions): string {
			...
			if (this._config.onResponse) {
				const resCtx: OnResponseContext = {
					...
					retry: () => {
						...
						return this._doRequest(method, path, opts, true, _requestMeta).then(async (r) => {
