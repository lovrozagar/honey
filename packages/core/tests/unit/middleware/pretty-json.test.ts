...
function makeApp(opts?: Parameters<typeof prettyJson>[0]) {
	...
	app
		.get("/data")
		.handler((ctx) => ctx.res.json("ok", { items: ["a", "b"], total: 2 }));
...
describe("pretty-json middleware", () => {
	it("?pretty → indented JSON output", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/data?pretty"),
			{},
		);
	...
	it("custom query param name (?format)", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/data?format"),
			{},
		);
		const text = await res.text();
		expect(text).toContain("\n");
	...
	it("non-JSON response → passthrough (no crash)", async () => {
		const app = makeApp();
		const res = await app.fetch(
			new Request("http://localhost/text?pretty"),
			{},
		);
	...
	it("preserves response status", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/created?pretty"),
			{},
		);
	...
	it("preserves response headers", async () => {
		...
		app
			.get("/h")
			.handler((ctx) =>
				ctx.res.json("ok", { x: 1 }, { headers: { "x-custom": "yes" } }),
			);
