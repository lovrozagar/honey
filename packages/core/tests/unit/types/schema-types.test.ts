...
describe("AllowedResMethods", () => {
	...
	it("all content types allows all methods", () => {
		...
		expectTypeOf<Methods>().toEqualTypeOf<
			"binary" | "csv" | "html" | "json" | "sse" | "text" | "xml"
		>();
	});
});
...
describe("ExtractSchemas", () => {
	it("extracts JSON schemas from output def", () => {
		type Schemas = ExtractSchemas<
			{ "application/json": { created: z.ZodString; ok: z.ZodNumber } },
			"application/json"
		>;
...
describe("UniversalResMethods", () => {
	it("contains exactly noContent, raw, redirect, stream", () => {
		expectTypeOf<UniversalResMethods>().toEqualTypeOf<
			"noContent" | "raw" | "redirect" | "stream"
		>();
	});
});
...
describe("MergeRoute", () => {
	...
	it("same path+method produces intersection (developer error scenario)", () => {
		...
		expectTypeOf<R2["/test"]["get"]["output"]>().toEqualTypeOf<
			{ a: true } & { b: true }
		>();
	});
