...
describe("interpolate: ICU plural", () => {
	...
	it("# replaced with actual number", () => {
		expect(interpolate("{n, plural, other {# results found}}", { n: 42 })).toBe(
			"42 results found",
		);
	});
	...
	it("plural falls back to other when category missing", () => {
		expect(interpolate("{count, plural, other {# items}}", { count: 1 })).toBe(
			"1 items",
		);
	});
...
describe("interpolate: ICU select", () => {
	it("select matches exact value", () => {
		expect(
			interpolate(
				"{role, select, admin {Administrator} user {Regular User} other {Unknown}}",
				{
					role: "admin",
				},
			),
	...
	it("select with missing var → empty string matches other", () => {
		expect(
			interpolate("{gender, select, male {He} female {She} other {They}}", {}),
		).toBe("They");
	});
...
describe("interpolate: backward compatibility", () => {
	it("simple var replacement still works", () => {
		expect(interpolate("{field} is required", { field: "Email" })).toBe(
			"Email is required",
		);
	});
...
describe("interpolate: edge cases", () => {
	...
	it("deeply nested plural in select", () => {
		expect(
			interpolate(
				"{type, select, summary {{count, plural, one {# item} other {# items}} total} other {details}}",
				{ count: 5, type: "summary" },
			),
		).toBe("5 items total");
	});
});
...
describe("interpolate: plural categories", () => {
	...
	it("=1 exact match takes precedence over one category", () => {
		expect(
			interpolate(
				"{n, plural, =1 {exact one} one {category one} other {many}}",
				{ n: 1 },
			),
		).toBe("exact one");
	});
	...
	it("=100 exact match", () => {
		expect(
			interpolate("{n, plural, =100 {century} other {# items}}", { n: 100 }),
		).toBe("century");
	});

	it("negative number → other category", () => {
		expect(
			interpolate("{n, plural, one {# item} other {# items}}", { n: -3 }),
		).toBe("-3 items");
	});

	it("fractional number → other category (not one)", () => {
		expect(
			interpolate("{n, plural, one {# item} other {# items}}", { n: 1.5 }),
		).toBe("1.5 items");
	});

	it("missing var in plural → defaults to 0", () => {
		expect(
			interpolate("{n, plural, =0 {zero} one {one} other {other}}", {}),
		).toBe("zero");
	});

	it("NaN as plural value → treated as 0", () => {
		expect(interpolate("{n, plural, =0 {zero} other {num}}", { n: NaN })).toBe(
			"num",
		);
	});
});
...
describe("interpolate: # replacement", () => {
	it("multiple # in same branch", () => {
		expect(interpolate("{n, plural, other {# out of # total}}", { n: 5 })).toBe(
			"5 out of 5 total",
		);
	});

	it("# not replaced outside plural context", () => {
		expect(interpolate("# is not replaced here", {})).toBe(
	...
	it("# with zero", () => {
		expect(interpolate("{n, plural, other {# items}}", { n: 0 })).toBe(
			"0 items",
		);
	});

	it("# with negative", () => {
		expect(interpolate("{n, plural, other {# offset}}", { n: -10 })).toBe(
			"-10 offset",
		);
	});
});
...
describe("interpolate: select exhaustive", () => {
	it("select with many branches", () => {
		const tpl =
			"{status, select, pending {Waiting} active {Running} done {Finished} error {Failed} other {Unknown}}";
	...
	it("select with numeric string value", () => {
		expect(
			interpolate("{x, select, 1 {One} 2 {Two} other {Other}}", { x: "1" }),
		).toBe("One");
	});

	it("select with numeric var → converted to string", () => {
		expect(
			interpolate("{x, select, 42 {Answer} other {Nope}}", { x: 42 }),
		).toBe("Answer");
	});

	it("select with empty string value → matches other", () => {
		expect(interpolate("{x, select, a {A} other {Empty}}", { x: "" })).toBe(
			"Empty",
		);
	});
...
describe("interpolate: deep nesting", () => {
	...
	it("three levels of nesting", () => {
		expect(
			interpolate(
				"{a, select, x {{b, select, y {{c}} other {nope}}} other {no}}",
				{
					a: "x",
					b: "y",
					c: "deep",
...
describe("interpolate: parser edge cases", () => {
	...
	it("consecutive ICU blocks no space", () => {
		expect(
			interpolate(
				"{a, plural, one {#} other {#s}}{b, plural, one {#} other {#s}}",
				{ a: 1, b: 2 },
			),
		).toBe("12s");
	});
	...
	it("literal text between ICU blocks", () => {
		expect(
			interpolate(
				"{a, plural, one {#} other {#s}} and {b, plural, one {#} other {#s}}",
				{
					a: 1,
					b: 2,
				},
...
describe("interpolate: real-world error messages", () => {
	it("validation: min length", () => {
		expect(
			interpolate(
				"{field} must be at least {min, plural, one {# character} other {# characters}} long",
				{
					field: "Password",
					min: 8,
				},
	...
	it("validation: max items", () => {
		expect(
			interpolate(
				"You can select {max, plural, one {up to # item} other {up to # items}}",
				{
					max: 1,
				},
			),
	...
	it("rate limit with window", () => {
		expect(
			interpolate(
				"Rate limited: {count}/{limit} {limit, plural, one {request} other {requests}} in {window} seconds",
				{ count: 150, limit: 100, window: 60 },
			),
