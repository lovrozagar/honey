...
describe("OpenAPI error schema — custom via errorFormatter(schema, mapper)", () => {
	it.skipIf(!hasZodJsonSchema)(
		"custom zod error schema replaces default in OpenAPI output",
		async () => {
	...
	it.skipIf(!hasZodJsonSchema)(
		"enum injection patches error_key property when present in custom schema",
		async () => {
	...
	it.skipIf(!hasZodJsonSchema)(
		"custom schema without error_key/status skips enum injection",
		async () => {
			const errors = defineErrors({ bad_input: "bad_request" });
...
describe("OpenAPI error schema — propagation", () => {
	it.skipIf(!hasZodJsonSchema)(
		"errorFormatter schema propagates through basePath()",
		async () => {
	...
	it.skipIf(!hasZodJsonSchema)(
		"errorFormatter schema propagates through use()",
		async () => {
			const errors = defineErrors({ not_found: "not_found" });
