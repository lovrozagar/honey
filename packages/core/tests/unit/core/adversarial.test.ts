...
describe("adversarial: malformed requests", () => {
	...
	it("extremely long query string → parsed without crash", async () => {
		const app = honey<{}>()
		app.get("/test").handler((ctx) => ctx.res.json("ok", { keys: Object.keys(ctx.search).length }))

		const longQuery = Array.from({ length: 1000 }, (_, i) => `k${i}=v${i}`).join("&")
		const res = await app.fetch(new Request(`http://localhost/test?${longQuery}`), {})
	...
	it("null byte in URL path → handled", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/test%00evil"), {})
	...
	it("null byte in query param → handled", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/test?q=hello%00world"), {})
		expect(res.status).toBe(200)
	})
...
describe("adversarial: error information leakage", () => {
	it("internal error → no stack trace in response", async () => {
		...
		app.get("/test").handler(() => {
			throw new Error("secret database connection string: postgres://user:pass@host/db")
		})
		...
		const app = honey<{}>()
			...
			.handler(() => {
				throw errors.db_error({
					cause: new Error("connection refused to postgres://admin:s3cr3t@prod-db:5432"),
				})
			})
...
describe("adversarial: timing attacks", () => {
	it("404 for different paths takes similar time", async () => {
		const app = honey<{}>()
		app.get("/secret-admin-panel").handler((ctx) => ctx.res.text("ok", "admin"))

		const times: number[] = []
		for (const path of ["/a", "/aaaaaaaaaa", "/secret-admin-pane", "/zzzzzzzzzzz"]) {
	...
	it("parallel requests to same route don't share mutable state", async () => {
		...
		const results = await Promise.all(
			Array.from({ length: 50 }, (_, i) =>
				app.fetch(new Request(`http://localhost/test?id=${i}`), {}).then(async (r) => {
