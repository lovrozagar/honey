...
describe("testClient: cookie jar", () => {
	it("collects Set-Cookie from response and sends on next request", async () => {
		...
		app
			.get("/login")
			.handler((ctx) => ctx.res.json("ok", { logged: true }, { cookies: { session: { value: "tok-123" } } }))
	...
	it("cookie jar disabled → cookies not forwarded", async () => {
		const app = honey<{}>()
		app.get("/login").handler((ctx) => ctx.res.json("ok", {}, { cookies: { session: { value: "tok" } } }))
		app.get("/me").handler((ctx) => ctx.res.json("ok", { session: ctx.cookies.session ?? "none" }))
	...
	it("multiple cookies collected across requests", async () => {
		const app = honey<{}>()
		app.get("/step1").handler((ctx) => ctx.res.json("ok", {}, { cookies: { a: { value: "1" } } }))
		app.get("/step2").handler((ctx) => ctx.res.json("ok", {}, { cookies: { b: { value: "2" } } }))
		app.get("/check").handler((ctx) => ctx.res.json("ok", { a: ctx.cookies.a, b: ctx.cookies.b }))
	...
	it("cookie overwritten by newer Set-Cookie", async () => {
		const app = honey<{}>()
		app.get("/set1").handler((ctx) => ctx.res.json("ok", {}, { cookies: { tok: { value: "old" } } }))
		app.get("/set2").handler((ctx) => ctx.res.json("ok", {}, { cookies: { tok: { value: "new" } } }))
		app.get("/check").handler((ctx) => ctx.res.json("ok", { tok: ctx.cookies.tok }))
...
describe("accepts: content negotiation", () => {
	...
	it("q=0 explicitly rejects type", () => {
		...
		expect(accepts(req, ["text/html", "application/json"])).toBe("application/json")
	})
	...
	it("multiple types with different q values", () => {
		...
		expect(accepts(req, ["text/html", "text/plain", "application/json"])).toBe("application/json")
	})
...
describe("response: cookie attributes", () => {
	it("httpOnly cookie → Set-Cookie contains HttpOnly", async () => {
		const app = honey<{}>()
		app.get("/test").handler((ctx) => ctx.res.json("ok", {}, { cookies: { sid: { httpOnly: true, value: "tok" } } }))
	...
	it("secure cookie → Set-Cookie contains Secure", async () => {
		const app = honey<{}>()
		app.get("/test").handler((ctx) => ctx.res.json("ok", {}, { cookies: { sid: { secure: true, value: "tok" } } }))
	...
	it("sameSite=strict → Set-Cookie contains SameSite=Strict", async () => {
		...
		app
			.get("/test")
			.handler((ctx) => ctx.res.json("ok", {}, { cookies: { sid: { sameSite: "strict", value: "tok" } } }))
	...
	it("maxAge → Set-Cookie contains Max-Age", async () => {
		const app = honey<{}>()
		app.get("/test").handler((ctx) => ctx.res.json("ok", {}, { cookies: { sid: { maxAge: 3600, value: "tok" } } }))
...
describe("response: content type methods", () => {
	...
	it("res.binary → application/octet-stream", async () => {
		const app = honey<{}>()
		app.get("/test").handler((ctx) => ctx.res.binary("ok", new Uint8Array([1, 2, 3])))
		const res = await app.fetch(new Request("http://localhost/test"), {})
	...
	it("onError can inspect and return custom response based on error type", async () => {
		const app = honey<{}>().onError((err) => {
			if (err instanceof Error && err.message === "db connection lost") {
				return new Response(JSON.stringify({ error: "service_unavailable", retry: true }), {
