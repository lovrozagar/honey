...
export async function extractBaseCtx(options: {
	...
	const tsConfigFilePath =
		options.tsconfigPath ?? findTsConfig(options.entryPath);
	...
	if (!varDecl) {
		throw new Error(
			`Export "${options.exportName}" not found in ${options.entryPath}`,
		);
	}
	...
	if (!ctxProp) {
		throw new Error(
			`Export "${options.exportName}" has no $ctx — not a Honey instance`,
		);
	}
...
export async function extractChainTypes(options: {
	...
	if (envSym) {
		envType = serialize(
			checker.getTypeOfSymbolAtLocation(envSym, baseNode),
			checker,
			baseNode,
			tsLib,
		);
	...
	const baseMiddlewareType = baseMwProps.length === 0 ? null
	...
	return {
		base: {
			envType: sanitize(envType),
			middlewareType: baseMiddlewareType
				? sanitize(baseMiddlewareType)
				: null,
			tapsType: tapsType ? sanitize(tapsType) : null,
		},
		routeMiddleware,
