...
import {
	...
	isErrorEnvelope,
	shortHash,
} from "../../../src/codegen-schema-naming.ts"
import { hashString } from "../../../src/codegen-hash.ts"

describe("deriveSchemaName", () => {
	it("POST collection → CreateXRequest", () => {
		expect(deriveSchemaName({ method: "post", path: "/v1/projects", role: "request" })).toBe("CreateProjectRequest")
	})

	it("POST collection 201 → CreateXResponse201", () => {
		expect(deriveSchemaName({ method: "post", path: "/v1/projects", role: "response", status: 201 })).toBe(
			"CreateProjectResponse201",
		)
	})

	it("POST collection 400 → CreateXResponse400", () => {
		expect(deriveSchemaName({ method: "post", path: "/v1/projects", role: "response", status: 400 })).toBe(
			"CreateProjectResponse400",
		)
	})

	it("GET collection → ListXsResponse200", () => {
		expect(deriveSchemaName({ method: "get", path: "/v1/projects", role: "response", status: 200 })).toBe(
			"ListProjectsResponse200",
		)
	})

	it("GET by id → GetXResponse200", () => {
		expect(deriveSchemaName({ method: "get", path: "/v1/projects/{id}", role: "response", status: 200 })).toBe(
			"GetProjectResponse200",
		)
	})

	it("PATCH by id → UpdateXRequest", () => {
		expect(deriveSchemaName({ method: "patch", path: "/v1/projects/{id}", role: "request" })).toBe(
	...
	it("POST sub-collection → CreateXsYRequest (intermediate plural not singularized — documented heuristic limit)", () => {
		...
		expect(deriveSchemaName({ method: "post", path: "/v1/projects/{id}/invites", role: "request" })).toBe(
			"CreateProjectsInviteRequest",
		)
	})
	...
	it("POST known-action → AcceptXYRequest (intermediate segments not singularized — documented heuristic limit)", () => {
		...
		expect(deriveSchemaName({ method: "post", path: "/v1/projects/{id}/invites/accept", role: "request" })).toBe(
			"AcceptProjectsInvitesRequest",
		)
	})
	...
	it("POST action-less leaf treated as resource → CreateAuthLoginRequest", () => {
		...
		expect(deriveSchemaName({ method: "post", path: "/v1/auth/login", role: "request" })).toBe("CreateAuthLoginRequest")
	})

	it("POST known action refresh → RefreshXRequest", () => {
		expect(deriveSchemaName({ method: "post", path: "/v1/auth/refresh", role: "request" })).toBe("RefreshAuthRequest")
	})
	...
	it("DELETE by id → DeleteXRequest", () => {
		expect(deriveSchemaName({ method: "delete", path: "/v1/projects/{id}", role: "request" })).toBe(
			"DeleteProjectRequest",
		)
	})

	it("PUT → ReplaceXRequest", () => {
		expect(deriveSchemaName({ method: "put", path: "/v1/projects/{id}", role: "request" })).toBe(
...
describe("deriveErrorEnvelopeName", () => {
	...
	it("2 keys a, b → Err400AB", () => {
		const schema = {
			properties: {
				error_key: { enum: ["a", "b"], type: "string" },
				status: { const: 400, type: "integer" },
				success: { const: false, type: "boolean" },
			},
			type: "object",
	...
	it("3 keys → concat pascal", () => {
		const schema = {
			properties: {
				error_key: { enum: ["bad_request", "invalid_input", "not_found"], type: "string" },
				status: { const: 400, type: "integer" },
				success: { const: false, type: "boolean" },
			},
			type: "object",
	...
	it("4 keys → concat all pascal (no truncation)", () => {
		const schema = {
			properties: {
				error_key: { enum: ["a", "b", "c", "d"], type: "string" },
				status: { const: 400, type: "integer" },
				success: { const: false, type: "boolean" },
			},
			type: "object",
	...
	it("5+ keys → concat all pascal, sorted", () => {
		const schema = {
			properties: {
				error_key: {
					enum: ["bad_gateway", "gateway_timeout", "internal_server_error", "service_unavailable", "bad_request"],
					type: "string",
				},
		...
		expect(deriveErrorEnvelopeName(schema)).toBe(
			"Err500BadGatewayBadRequestGatewayTimeoutInternalServerErrorServiceUnavailable",
		)
	})
	...
	it("unsorted input → same name as sorted (order independence)", () => {
		...
		const reversed = {
			properties: {
				error_key: { enum: ["b", "a"], type: "string" },
				status: { const: 400, type: "integer" },
				success: { const: false, type: "boolean" },
			},
			type: "object",
