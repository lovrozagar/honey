...
function zodIsOptional(def: Record<string, unknown>): boolean {
	...
	if (t === "ZodNullable" || t === "nullable" || t === "ZodDefault" || t === "default" || t === "ZodCatch" || t === "catch") {
...
function emitZod(schema: unknown): string {
	...
	switch (typeName) {
		...
		case "literal": {
			...
			return values
				.map((v0) => (typeof v0 === "string" ? `"${v0}"` : String(v0)))
				.join(" | ");
		}
		case "ZodEnum":
		case "enum": {
...
function emitValibot(schema: unknown): string {
	...
	switch (type) {
		...
		case "object": {
			...
			return `{ ${keys
				.map((k) => {
					const propType = (entries[k] as Record<string, unknown>).type as
						| string
						| undefined;
...
function isBooleanUnion(node: unknown[]): boolean {
	...
	return (
		(units[0] === false && units[1] === true) ||
		(units[0] === true && units[1] === false)
	);
}
	...
	if ("domain" in obj) {
		...
		if (domain === "object") {
			...
			for (const entry of required) {
				parts.push(
					`${quoteKey(String(entry.key))}: ${emitArkJson(entry.value)}`,
				);
			}
