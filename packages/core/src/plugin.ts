...
export async function generateAndWrite(config: ResolvedHoneyConfig, root: string): Promise<void> {
...
export function honey(config: HoneyVitePluginConfig) {
	...
	const plugin: {
		...
		transform(code: string, id: string): { code: string; map: null } | undefined {
			...
			if (/(?<!Bun)(?<!Deno)\.serve\s*\(/.test(code) && !code.includes("@lovrozagar/honey/serve")) {
