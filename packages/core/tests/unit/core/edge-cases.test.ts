...
describe("throw non-Error values", () => {
	it("throw string → 500 internal_server_error", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/throw-string"), {})
		expect(res.status).toBe(500)
		const body = (await res.json()) as Record<string, unknown>
		expect(body.error_key).toBe("internal_server_error")
	})
	...
	it("throw number → 500 internal_server_error", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/throw-number"), {})
		expect(res.status).toBe(500)
	})
...
describe("optional param at root", () => {
	it("/:lang? matches both / and /en", async () => {
		const app = honey<{}>()
		app.get("/:lang?").handler((ctx) => ctx.res.json("ok", { lang: ctx.params.lang ?? "default" }))
...
describe("HEAD auto-handling with ALL handler", () => {
	it("HEAD to route with only ALL handler → 200, empty body", async () => {
		const app = honey<{}>()
		app.all("/catch-all").handler((ctx) => ctx.res.json("ok", { method: ctx.req.method }))

		const res = await app.fetch(new Request("http://localhost/catch-all", { method: "HEAD" }), {})
		expect(res.status).toBe(200)
