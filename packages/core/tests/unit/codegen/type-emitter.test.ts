...
describe("emitSchemaType", () => {
	...
	describe("zod — objects", () => {
		...
		it("nullable optional property emits ?", () => {
			...
			expect(emitSchemaType(schema)).toBe(
				"{ bio?: string | null | undefined; name: string }",
			);
		});
	...
	describe("zod — optionals and nullables", () => {
		...
		it("optional nullable", () => {
			expect(emitSchemaType(z.string().nullable().optional())).toBe(
	...
	describe("zod — unions and intersections", () => {
		...
		it("discriminated union", () => {
			...
			expect(emitSchemaType(schema)).toBe(
				'{ type: "a"; val: string } | { type: "b"; val: number }',
			);
		});
	...
	describe("zod — wrappers", () => {
		...
		it("readonly", () => {
			expect(emitSchemaType(z.object({ a: z.string() }).readonly())).toBe(
				"Readonly<{ a: string }>",
			);
		});
	...
	describe("valibot — optionals and nullables", () => {
		...
		it("optional nullable", () => {
			expect(emitSchemaType(v.optional(v.nullable(v.string())))).toBe(
				"string | null | undefined",
			);
		});
	});
	...
	describe("valibot — enums", () => {
		it("string enum", () => {
			expect(emitSchemaType(v.picklist(["a", "b", "c"]))).toBe(
				'"a" | "b" | "c"',
			);
		});
	});
	...
	describe("valibot — records and tuples", () => {
		it("record", () => {
			expect(emitSchemaType(v.record(v.string(), v.number()))).toBe(
				"Record<string, number>",
			);
		});
