...
describe("emitSchemaType — edge cases", () => {
	...
	describe("valibot — nullish", () => {
		it("nullish string", () => {
			expect(emitSchemaType(v.nullish(v.string()))).toBe(
	...
	describe("valibot — optional with complex inner", () => {
		it("optional array of numbers", () => {
			expect(emitSchemaType(v.optional(v.array(v.number())))).toBe(
				"number[] | undefined",
			);
		});
	});
	...
	describe("valibot — deeply nested objects", () => {
		it("object with array of objects containing nested arrays", () => {
			...
			expect(emitSchemaType(schema)).toBe(
				"{ users: { name: string; tags: string[] }[] }",
			);
		});
	});
	...
	describe("arktype — union including null", () => {
		it("string | number | null", () => {
			...
			expect(emitSchemaType(type("string | number | null"))).toBe(
				"number | string | null",
			);
		});
	});
	...
	describe("cross-vendor consistency", () => {
		...
		it("object with optional property across all vendors", () => {
			const zResult = emitSchemaType(
				z.object({ age: z.number().optional(), name: z.string() }),
			);
			const vResult = emitSchemaType(
				v.object({ age: v.optional(v.number()), name: v.string() }),
			);
			const aResult = emitSchemaType(
		...
		it("object with nullable (non-optional) property — no ? across vendors", () => {
			const zResult = emitSchemaType(
				z.object({ avatar: z.string().nullable() }),
			);
			const vResult = emitSchemaType(
				v.object({ avatar: v.nullable(v.string()) }),
			);
		...
		it("object with nullable+optional property across all vendors", () => {
			const zResult = emitSchemaType(
				z.object({ bio: z.string().nullable().optional() }),
			);
			const vResult = emitSchemaType(
				v.object({ bio: v.optional(v.nullable(v.string())) }),
			);
